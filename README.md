...
`tools/list` at `https://gateway.pipeworx.io/jikan/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/jikan_search_anime \
  -H 'Content-Type: application/json' \
  -d '{"query":"Fullmetal Alchemist"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/jikan_search_anime`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
